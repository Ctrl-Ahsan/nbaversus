...
const Stats = (props) => {
    ...
    useEffect(() => {
        const fetchStats = () => {
            ...
            Promise.all([
                fetch(`https://data.nba.net/data/10s/prod/v1/2022/players/${props.p1.personId}_profile.json`).then((value) => value.json()),
                fetch(`https://data.nba.net/data/10s/prod/v1/2022/players/${props.p2.personId}_profile.json`).then((value) => value.json())
            ])
            .then(([json1, json2]) => {
                setStats1(json1)
                setStats2(json2)
